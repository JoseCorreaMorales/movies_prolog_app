    pelicula(
    % Id
    "11", 
    % Género 
    "Thriller, Drama, Misterio",
    % Nombre
    "El sexto sentido",
    % Actores
    "Bruce Willis, Haley Joel Osment, Toni Collette",
    % Sinopsis
    "El psicólogo infantil Malcolm Crowe trata de ayudar a un niño que afirma ver personas muertas. A medida que Malcolm profundiza en el caso, empieza a dudar de su propia cordura y de la realidad que le rodea.",
    % Duración
    "107 minutos",
    % Idioma
    "Inglés",
    % Año
    1999,
    % Enlace (donde puedo verla)
    "https://www.primevideo.com/detail/El-Sexto-Sentido/0G9SO4O0U3110GKI50S4CJXY6B",
    % Imagen (URL de la portada)
    "https://www.ecartelera.com/carteles/5400/5409/001_p.jpg"
     ).


    pelicula( 
    % Id
    "12", 
    % Género 
    "Thriller, Misterio, Crimen",
    % Nombre
    "El curioso caso de Benjamin Button",
    % Actores
    "Brad Pitt, Cate Blanchett, Taraji P. Henson",
    % Sinopsis
    "La historia sigue a un hombre que nace con 80 años y va rejuveneciendo con el paso del tiempo. A lo largo de su vida, Benjamin Button experimenta amor, pérdida y una serie de eventos extraordinarios y misteriosos que desafían la comprensión humana.",
    % Duración
    "166 minutos",
    % Idioma
    "Inglés",
    % Año
    2008,
    % Enlace (donde puedo verla)
    "https://www.amazon.es/gp/video/detail/B00I3MXQVO/ref=atv_dp_share_cu_r",
    % Imagen (URL de la portada)
    "https://www.funeralnatural.net/sites/default/files/pelicula/imagen/el_curioso_caso_de_benjamin.jpg"
     ).

    pelicula(
    % Id
    "13", 
    % Género 
    "Thriller, Drama, Crimen",
    % Nombre
    "Pulp Fiction",
    % Actores
    "John Travolta, Samuel L. Jackson, Uma Thurman, Harvey Keitel",
    % Sinopsis
    "Un par de asesinos a sueldo, un boxeador, un gángster y su esposa, y un par de ladrones se entrelazan en varias historias no lineales en Los Ángeles.",
    % Duración
    "154 minutos",
    % Idioma
    "Inglés",
    % Año
    1994,
    % Enlace (donde puedo verla)
    "https://www.netflix.com/title/880039",
    % Imagen (URL de la portada)
    "https://static.posters.cz/image/1300/posters/pulp-fiction-cover-i1288.jpg"
    ).

    pelicula(
    % Id
    "14", 
    % Género 
    "Thriller, Drama, Crimen",
    % Nombre
    "The Silence of the Lambs",
    % Actores
    "Jodie Foster, Anthony Hopkins, Scott Glenn",
    % Sinopsis
    "Una joven agente del FBI busca la ayuda de un psiquiatra manipulador y peligroso para capturar a un asesino en serie que desolla a sus víctimas.",
    % Duración
    "118 minutos",
    % Idioma
    "Inglés",
    % Año
    1991,
    % Enlace (donde puedo verla)
    "https://www.amazon.es/El-Silencio-los-Corderos/dp/B00GYLR452",
    % Imagen (URL de la portada)
    "https://m.media-amazon.com/images/M/MV5BMWE3NTE2YzgtNjUyYy00Y2NmLTljNWMtNjY4MjZhYmMzM2M4XkEyXkFqcGdeQXVyMTEyMDcwNw@@._V1_.jpg"
     ).


    pelicula(
    % Id
    "15", 
    % Género 
    "Thriller, Drama, Misterio",
    % Nombre
    "Gone Girl",
    % Actores
    "Ben Affleck, Rosamund Pike, Neil Patrick Harris, Tyler Perry",
    % Sinopsis
    "La desaparición de Amy Dunne lleva a una investigación policial y mediática que revela secretos oscuros sobre su matrimonio y su esposo Nick Dunne, llevando a una sorprendente conclusión.",
    % Duración
    "149 minutos",
    % Idioma
    "Inglés",
    % Año
    2014,
    % Enlace (donde puedo verla)
    "https://www.hbo.com/movies/gone-girl",
    % Imagen (URL de la portada)
    "https://cdnb.artstation.com/p/assets/images/images/027/498/209/large/amin-lashgari-02-gone-girl.jpg?1591714763"
    ).

    pelicula(
    % Id
    "16", 
    % Género 
    "Drama, Crimen",
    % Nombre
    "The Shawshank Redemption",
    % Actores
    "Tim Robbins, Morgan Freeman",
    % Sinopsis
    "Andy Dufresne, un banquero condenado por un crimen que no cometió, es enviado a la Penitenciaría Estatal de Shawshank donde desarrolla una amistad única con Red mientras lucha por su libertad.",
    % Duración
    "142 minutos",
    % Idioma
    "Inglés",
    % Año
    1994,
    % Enlace (donde puedo verla)
    "https://www.netflix.com/title/70005379",
    % Imagen (URL de la portada)
    "https://m.media-amazon.com/images/M/MV5BNDE3ODcxYzMtY2YzZC00NmNlLWJiNDMtZDViZWM2MzIxZDYwXkEyXkFqcGdeQXVyNjAwNDUxODI@._V1_FMjpg_UX1000_.jpg"
    ).

    pelicula(
    % Id
    "17", 
    % Género 
    "Ciencia Ficción, Drama, Romance",
    % Nombre
    "Her",
    % Actores
    "Joaquin Phoenix, Amy Adams, Rooney Mara, Scarlett Johansson (voz)",
    % Sinopsis
    "Theodore Twombly se enamora del sistema operativo avanzado que ha personalizado para adaptarse a su vida, explorando temas como la soledad, la tecnología y la conexión humana.",
    % Duración 
    "126 minutos",
    % Idioma
    "Inglés",
    % Año
    2013,
    % Enlace (donde puedo verla)
    "https://www.hbo.com/movies/her",
    % Imagen (URL de la portada)
    "https://scifiagenda.com/img/posters/her.jpg"
    ).

    pelicula(
    % Id
    "18",    
    % Género
    "Drama, Romance, Musical",
    % Nombre
    "La La Land",
    % Actores
    "Emma Stone, Ryan Gosling",
    % Sinopsis
    "Mia y Sebastian son dos jóvenes creativos que aspiran a triunfar en Los Ángeles y se enamoran mientras luchan por sus sueños en una ciudad implacable.",
    % Duración
    "128 minutos",
    % Idioma
    "Inglés",
    % Año
    2016,
    % Enlace (donde puedo verla)
    "https://www.netflix.com/title/80095241",
    % Imagen (URL de la portada)
    "https://m.media-amazon.com/images/I/51si8ItHWrL._AC_UF894,1000_QL80_.jpg"
    ).


    pelicula(
    % Id
    "19",
    % Género
    "Drama, Crimen",
    % Nombre
    "The Godfather",
    % Actores
    "Marlon Brando, Al Pacino, James Caan, Robert Duvall",
    % Sinopsis
    "La familia mafiosa Corleone lucha por mantener su poder y proteger a sus miembros en medio de la violencia y el crimen en Nueva York.",
    % Duración
    "175 minutos",
    % Idioma
    "Inglés",
    % Año
    1972,
    % Enlace (donde puedo verla)
    "https://www.netflix.com/title/238413",
    % Imagen (URL de la portada)
    "https://m.media-amazon.com/images/M/MV5BM2MyNjYxNmUtYTAwNi00MTYxLWJmNWYtYzZlODY3ZTk3OTFlXkEyXkFqcGdeQXVyNzkwMjQ5NzM@._V1_.jpg"
    ).

    pelicula(
    % Id
    "20",
    % Género
    "Ciencia ficción, Drama",
    % Nombre
    "Interstellar",
    % Actores
    "Matthew McConaughey, Anne Hathaway, Jessica Chastain, Michael Caine",
    % Sinopsis
    "Un equipo de astronautas viaja a través de un agujero de gusano en busca de un nuevo hogar para la humanidad.",
    % Duración
    "169 minutos",
    % Idioma
    "Inglés",
    % Año
    2014,
    % Enlace (donde puedo verla)
    "https://www.netflix.com/title/70305903",
    % Imagen (URL de la portada)
    "https://w0.peakpx.com/wallpaper/968/682/HD-wallpaper-interstellar-movie-thumbnail.jpg"
     ).

%------------------------------------------------------------RULES-----------------------------------------------------%

thriller() :- pelicula(Id, Genero, Nombre, Actores, Sinopsis, Duracion, Idioma, Fecha, Enlace, Imagen),
               write(Id), nl,
               write(Genero), nl,
               write(Nombre), nl,
               write(Actores), nl,
               write(Sinopsis), nl,
               write(Duracion), nl,
               write(Idioma), nl,
               write(Fecha), nl,
               write(Enlace), nl,
               write(Imagen), nl, fail.